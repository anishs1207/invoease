...
const registerUser = asyncHandler(async (req, res, next) => {
  ...
  if (!fullName || !email || !username || !password) {
    throw new ApiError({ statusCode: 400, message: "some fields are missing" });
  }
  ...
  if (existingUser) {
    if (existingUser.isOAuthUser) {
      return res.status(409).json({
        success: false,
        message:
          "User already registered via Google. Please login using Google.",
      });
    }
  ...
  const createdUser = await User.findById(user._id).select(
    "username email fullName createdAt updatedAt"
  );
...
const verifyCode = asyncHandler(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { isVerified: true }, "Verification successful!")
    );
});
...
const getUserSession = asyncHandler(async (req, res) => {
  ...
  const user = await User.findById(req.user._id).select(
    "username email fullName isVerified createdAt updatedAt"
  );
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { user }, "User session retrieved successfully")
    );
});
