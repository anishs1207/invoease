...
const getInvoices = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, userInvoices, "Invoices retrieved successfully")
      );
  } catch (error) {
    console.error("Error fetching invoices:", error);
    return res
      .status(500)
      .json(new ApiResponse(500, null, "Failed to retrieve invoices"));
  }
});
...
const deleteInvoice = asyncHandler(async (req, res) => {
  ...
  if (!id) {
    throw new ApiError(400, "Id is not passed");
  }
