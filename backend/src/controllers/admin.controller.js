...
export const sendNotifications = asyncHandler(async (req, res) => {
  ...
  if (sender.role !== "admin") {
    throw new ApiError(
      403,
      "Access Denied: Only admins can send notifications."
    );
  }
...
export const deleteNotificationByText = asyncHandler(async (req, res) => {
  ...
  const result = await User.updateMany(
    {},
    { $pull: { notifications: { alertText: alertText } } }
  );
    ...
    return res
      ...
      .json(
        new ApiResponse(
          404,
          null,
          "No notifications found with the given text."
        )
      );
  }
...
export const updateNotificationById = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, null, "Notification updated successfully."));
});
